                    ...
                    if(widget.charts[i].chartType == 'line' || 'bar'){
                        if(typeof(widget.charts[i].chartData[0].total) === 'object') {
                            ...
                            for(j=0;j<widget.charts[i].chartData.length;j++){
                                ...
                                formattedChartData.push({x: inputDate, y:widget.charts[i].chartData[j].total[endpoint]});
                            }
                            ...
                            for(j=0;j<widget.charts[i].chartData.length;j++){
                                ...
                                newDate = splitDate[1]+'/'+splitDate[2]+'/'+splitDate[0];
                                inputDate = new Date(newDate).getTime();
                                formattedChartData.push({x: inputDate, y:widget.charts[i].chartData[j].total});
                            }
                            changedWidget.charts[i].chartData = formattedChartData;
                        }
                    }
        ...
        this.formatDataPoints = function(widget) {
            ...
            for(i=0;i<widget.charts.length;i++){
                if(widget.charts[i].chartType == 'line' || 'bar'){
                    ...
                    changedWidget = formatLineBarChartData(widget.charts[i].chartData,widget.charts[i].metricDetails,widget.charts[i].chartObjectTypeId);
                    /*formattedValues.push(formatValues(widget.charts[i].chartData,widget.charts[i].metricDetails,widget.charts[i].chartObjectTypeId));
                    formattedDates.push(formatDates(widget.charts[i].chartData));
                    widget.charts[i].chartData = [];
                    for(j=0;j<formattedDates.length;j++){
                        widget.charts[i].chartData.push([formattedDates[j],formattedValues[j]]);
                    }*/
                } else {
                    console.log('In the else condition. Needs to be addressed as chart type is different');
                }
            ...
            function formatLineBarChartData(chartData,metricDetails,objectTypeId){
                var formattedValues = [];
                if(typeof(chartData[0].total) === 'object')
                {
                    var endpoint;
                    for(i=0;i<metricDetails.objectTypes.length;i++){
                        if(metricDetails.objectTypes[i].objectTypeId == objectTypeId){
                            endpoint = metricDetails.objectTypes[i].meta.endpoint;
                        }
                    }
                    for(i=0;i<chartData.length;i++){
                        var splitDate, newDate, inputDate;
                        splitDate = chartData[i].date;
                        newDate = splitDate[1]+'/'+splitDate[2]+'/'+splitDate[0];
                        inputDate = new Date(newDate).getTime();
                        formattedValues.push({x: inputDate, y:chartData[i].total[endpoint]});
                    }
                }else{
                    for(i=0;i<chartData.length;i++){
                        var splitDate, newDate, inputDate;
                        ...
                        newDate = splitDate[1]+'/'+splitDate[2]+'/'+splitDate[0];
                        inputDate = new Date(newDate).getTime();
                        formattedValues.push({x: inputDate, y:chartData[i].total});
                    }
                }
            }

            function formatDates(chartData){
                var splitDate, newDate, inputDate;
                var formattedDates = [];
                for(i=0;i<chartData.length;i++){
                    splitDate = chartData[i].date;
                    newDate = splitDate[1]+'/'+splitDate[2]+'/'+splitDate[0];
                    inputDate = new Date(newDate).getTime();
                    formattedDates.push(inputDate);
                }
                return formattedDates;
            }

            function formatValues(chartData,metricDetails,objectTypeId){
                var formattedValues = [];
                if(typeof(chartData[0].total) === 'object')
                {
                    console.log('Inside object loop');
                    var endpoint;
                    for(i=0;i<metricDetails.objectTypes.length;i++){
                        if(metricDetails.objectTypes[i].objectTypeId == objectTypeId){
                            endpoint = metricDetails.objectTypes[i].meta.endpoint;
                    ...
                    for(i=0;i<chartData.length;i++){
                        formattedValues.push(chartData[i].total[endpoint]);
                    }
                } else {
                    console.log('Inside string loop');
                    for(i=0;i<chartData.length;i++){
                        formattedValues.push(chartData[i].total);
                    }
                }
...
showMetricApp.service('dashboardService', function(){
        return {
            printVariable: "hello",
            displayWidgetDataInGraph: function (graphData){
...
showMetricApp.factory ('generator',function(){
        return {
            lineChart: {
                options: function(){
                    return {
