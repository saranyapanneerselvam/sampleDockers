...
var AdWordsObject = require('../adwordsCustomerObject');
...
function AdWordsService(options) {
    ...
    self.formGetRequest = function(selector) {
        ...
        if (_.keys(getMethod.input).indexOf('selector') > -1) {
            request.selector = JSON.parse(selector);
        } else if (_.keys(getMethod.input).indexOf('serviceSelector') > -1) {
            request.serviceSelector = JSON.parse(selector);
        }
            ...
            function(cb) {
                if (self.client) {
                    ...
                    soap.createClient(self.wsdlUrl, function(err, client) {
                        console.log('client',self.wsdlUrl,client)
                        self.client = client;
    ...
    self.getCustomer = function(accessToken,done) {
        console.log('customerid')
        self.soapHeader.RequestHeader.accessToken = accessToken;
        ...
        async.waterfall([
            ...
            function(err, response) {
                console.log('responsecustomerservice',response)
